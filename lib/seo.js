...
export const SITE_URL = "https://www.promptforums.org";
...
export function socialMetadata({ title, description, url, type = "website" }) {
  return {
    openGraph: {
      title,
      description,
  ...
  return {
    ...
    itemListElement: sites.map((site, i) => {
      ...
      if (
        ...
        site.rating > 0 &&
        Number.isInteger(site.reviewsCount) &&
        site.reviewsCount > 0
      ) {
